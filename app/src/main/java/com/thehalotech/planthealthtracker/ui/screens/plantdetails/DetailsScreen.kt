...
@Composable
fun MainData(p : MyPlantsTable, navController: NavController, viewModel: PlantDetailsViewModel) {

    val scrollBehavior = TopAppBarDefaults.enterAlwaysScrollBehavior()
    val plantName = p.plantName
    ...
    val daysUntilWater = daysUntilWater(lastWatered, wateringFrequency.toInt())
    val lastWateredDate = epochToDate(lastWatered)


    val moistureLevel = when(wateringFrequency) {
        "1" ->  80
        "2" -> 60
        "3" -> 40
        "7" -> 20
        ...
        LazyColumn(modifier = Modifier.padding(padding)) {
            ...
            item {
                Column(modifier = Modifier.offset(y = (-80).dp)) {
                    ...
                    HorizontalDivider(thickness = 2.dp, modifier = Modifier.padding(10.dp))
                    Spacer(modifier = Modifier.height(20.dp))
                    Button(onClick = {onWaterClicked(p.plantName, viewModel)},
